...
// 하나의 프로세스가 가질수 있는 스레드의 개수는 제한되어 있지 않으나
// 스레드가 작업을 수행하는데 개별적인 메모리 공간(호출 스택)을 필요로 하기 떄문에
...
// 여러 사용자에게 서비스를 해주는 서버 프로그램의 경우 멀티스레드로 작겅하는 것이 필수적이어서
// 하나의 서버 프로세스가 여러 개의 스레드를 생성해서 스레드와 사용자의 요청이 일대일로 처리되도록
// 프로그래밍 해야한다.
// 만일 싱글 스래드로 서버 프로그램을 작성한다면 사용자의 요청 마다 새로운 프로세스를 생성 해야 하는데
// 프로세스를 생성하는 것은 스레드를 생성하는 것에 비해 더많은 시간과 메모리 공간이 필요하기 때문에
// 많은 수의 사용자 요정을 서비스하기 어렵다

// 그러나 멀티스레드 프로세스는 여러 스레드가 같은 프로세스 내에서 자원을 공유하면서
// 작업을 하기 떄문에 발생할 수 있는 동기화, 교착상태와 같은 문제들을 고려해서 신중하게 프로그래밍해야한다.
...
// -> 어느쪽을 선택해도 별차이는 없지만, Thread클래스를 상소
...
class Thread_runable extends Thread {
    @Override
    public void run() {
        for(int i =0; i < 10; ++i);
        System.out.println();
